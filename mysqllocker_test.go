...
func TestLock(t *testing.T) {
	t.Run("locks", func(t *testing.T) {
		...
		defer cancel()
		errs, err := Lock(ctx, db, lockName, time.Millisecond, 0)
		require.NoError(t, err)
	...
	t.Run("can't get the same lock twice", func(t *testing.T) {
		...
		ctx, cancel := context.WithCancel(context.Background())
		defer cancel()
		_, err := Lock(ctx, db, lockName, time.Second, 0)
		require.NoError(t, err)
		errs, err := Lock(ctx, db, lockName, time.Second, 0)
		require.Error(t, err)
		require.Nil(t, errs)
	})
	...
	t.Run("times out waiting for lock", func(t *testing.T) {
		...
		startTime := time.Now()
		errs, err := Lock(ctx, db, lockName, time.Second, timeout)
	...
	t.Run("release and relock", func(t *testing.T) {
		...
		defer cancel2()
		errs, err = Lock(ctx2, db, lockName, time.Second, 0)
		require.NoError(t, err)
		require.NotNil(t, errs)
		cancel2()
		require.NoError(t, <-errs)
